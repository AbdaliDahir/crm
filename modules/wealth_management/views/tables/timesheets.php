...
$aColumns = hooks()->apply_filters('patrimoines_timesheets_table_sql_columns', $aColumns);
...
$join = hooks()->apply_filters('patrimoines_timesheets_table_sql_join', $join);

$where = ['AND task_id IN (SELECT id FROM ' . db_prefix() . 'tasks WHERE rel_id="' . $this->ci->db->escape_str($patrimoine_id) . '" AND rel_type="patrimoine")'];

if (!has_permission('patrimoines', '', 'create')) {
    array_push($where, 'AND ' . db_prefix() . 'taskstimers.staff_id=' . get_staff_user_id());
}

$staff_ids = $this->ci->patrimoines_model->get_distinct_tasks_timesheets_staff($patrimoine_id);
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($i == 0) {
            ...
            if ($user_removed_as_assignee == 1) {
                $_data .= '<span class="hidden"> - </span> <span class="mtop5 pull-right" data-toggle="tooltip" data-title="' . _l('patrimoine_activity_task_assignee_removed') . '"><i class="fa fa-exclamation-circle" aria-hidden="true"></i></span>';
            }
            $_data .= '</div>';
    ...
    $options        = '';
    if (($aRow['staff_id'] == get_staff_user_id() || has_permission('patrimoines', '', 'edit'))) {
        if (($aRow['staff_id'] == get_staff_user_id() || has_permission('patrimoines', '', 'edit'))) {
        ...
        if (has_permission('patrimoines', '', 'delete') || has_permission('tasks', '', 'delete') || $aRow['staff_id'] == get_staff_user_id()) {

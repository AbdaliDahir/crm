...
foreach ($milestones as $milestone) {
  ...
  $cpicker = '';
  if (staff_can('edit_milestones', 'patrimoines') && $milestone['id'] != 0) {
  ...
  $total_pages = ceil($this->patrimoines_model->do_milestones_kanban_query($milestone['id'], $patrimoine_id, 1, $milestonesTasksWhere, true) / get_option('tasks_kanban_limit'));

  $tasks = $this->patrimoines_model->do_milestones_kanban_query($milestone['id'], $patrimoine_id, 1, $milestonesTasksWhere);
...
?>
  <ul class="kan-ban-col milestone-column<?php if (!staff_can('edit_milestones', 'patrimoines') || $milestone['id'] == 0) {
    ...
    <li class="kan-ban-col-wrapper">
      <div class="border-right panel_s">
        <div class="panel-heading panel-heading-bg <?php if ($milestone_color != '') {
          ...
          <?php if ($milestone['id'] != 0 && (staff_can('create', 'tasks') || staff_can('edit_milestones', 'patrimoines'))) { ?>
   ...
   <?php if (staff_can('edit_milestones', 'patrimoines')) { ?>
   <?php if ($cpicker != '') {
                echo '<hr />';
              }; ?>
     ...
     <?php echo _l('reset_to_default_color'); ?>
        ...
        <div class="kan-ban-content-wrapper">
          <div class="kan-ban-content">
            <ul class="status patrimoine-milestone milestone-tasks-wrapper sortable relative" data-task-status-id="<?php echo $milestone['id']; ?>">
              <?php
              foreach ($tasks as $task) {
                $this->load->view('wealth_management/_milestone_kanban_card', array('task' => $task, 'milestone' => $milestone['id']));
              } ?>
              <?php if ($total_tasks > 0) { ?>
                <li class="text-center not-sortable kanban-load-more" data-load-status="<?php echo $milestone['id']; ?>">
                  <a href="#" class="btn btn-default btn-block<?php if ($total_pages <= 1) {
                                                                echo ' disabled';
                                                              } ?>" data-page="1" onclick="kanban_load_more(<?php echo $milestone['id']; ?>,this,'wealth_management/milestones_kanban_load_more',320,360); return false;" ;>

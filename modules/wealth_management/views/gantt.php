			...
			<div class="panel_s">
				<div class="panel-body">
					<div class="row">
						...
						<?php echo form_open(admin_url('wealth_management/gantt'), ['method' => 'get', 'id' => 'ganttFiltersForm']); ?>
						<div class="col-md-4">
						...
						<?php
						/**
						 * Only show this filter if user has permission for patrimoines view otherwise
						 * wont need this becuase by default this filter will be applied
						 */
						if (has_permission('patrimoines', '', 'view')) {
						?>
							<div class="col-md-3">
								<select class="selectpicker" data-live-search="true" data-title="<?php echo _l('patrimoine_member'); ?>" name="member" data-width="100%">
									<option value=""></option>
									<?php foreach ($patrimoine_members as $member) { ?>
		...
		if (gantt_data.length > 0) {
			var gantt = new Gantt("#gantt", gantt_data, {
				...
				on_click: function(data) {
					if (typeof(data.patrimoine_id) != 'undefined') {
						var patrimoineViewUrl = '<?php echo admin_url('wealth_management/view'); ?>';
						window.location.href = patrimoineViewUrl + '/' + data.patrimoine_id;

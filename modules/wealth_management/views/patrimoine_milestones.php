...
<?php if(staff_can('create_milestones', 'patrimoines')){ ?>
...
<?php if($milestones_found) { ?>
   <div id="kanban-params" class="pull-right">
      <div class="checkbox">
         <input type="checkbox" value="yes" id="exclude_completed_tasks" name="exclude_completed_tasks"<?php if($milestones_exclude_completed_tasks){echo ' checked';} ?> onclick="window.location.href = '<?php echo admin_url('wealth_management/view/'.$patrimoine->id.'?group=patrimoine_milestones&exclude_completed='); ?>'+(this.checked ? 'yes' : 'no')">
...
<?php } ?>
<?php if($milestones_found){ ?>
   <div class="patrimoine-milestones-kanban">

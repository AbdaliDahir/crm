...
class Patrimoine_reports extends AdminController
{
    ...
    {
        $this->load->model('reports/knowledge_base_model');
        $data['groups'] = $this->knowledge_base_model->get_kbg();
        $data['title'] = _l('kb_reports');
        $this->load->view('reports/knowledge_base_articles', $data);
    }
    ...
    $this->load->view('reports/tax_summary',$data);
    }*/
    ...
    public function leads()
    {
        ...
        if ($this->input->get('type')) {
            ...
            $data['leads_staff_report'] = json_encode($this->patrimoine_reports_model->leads_staff_report());
        }
        $this->load->model('leads_model');
        $data['statuses'] = $this->leads_model->get_status();
        $data['leads_this_week_report'] = json_encode($this->patrimoine_reports_model->leads_this_week_report());
        $data['leads_sources_report'] = json_encode($this->patrimoine_reports_model->leads_sources_report());
    ...
    public function sales()
    {
        ...
        $data['payments_years'] = $this->patrimoine_reports_model->get_distinct_payments_years();
    ...
    public function expenses($type = 'simple_report')
    {
        ...
        if ($type != 'simple_report') {
            ...
            $this->load->view('reports/expenses_detailed', $data);
        } else {
            ...
            $data['chart_not_billable'] = json_encode($this->patrimoine_reports_model->get_stats_chart_data(_l('not_billable_expenses_by_categories'), [
                'billable' => 0,
            ], [
            ...
            $data['chart_billable'] = json_encode($this->patrimoine_reports_model->get_stats_chart_data(_l('billable_expenses_by_categories'), [
                'billable' => 1,
            ], [
    ...
    public function expenses_vs_income($year = '')
    {
        ...
        $payments_years = $this->patrimoine_reports_model->get_distinct_payments_years();
        ...
        $data['years'] = $_years;
        $data['chart_expenses_vs_income_values'] = json_encode($this->patrimoine_reports_model->get_expenses_vs_income_report($year));
    ...
    {
        echo json_encode($this->patrimoine_reports_model->total_income_report());
    }

    public function report_by_payment_modes()
    {
        echo json_encode($this->patrimoine_reports_model->report_by_payment_modes());
    }

    public function report_by_customer_groups()
    {
        echo json_encode($this->patrimoine_reports_model->report_by_customer_groups());
    }

    /* Leads conversion monthly report / ajax chart*/
    public function leads_monthly_report($month)
    {
        echo json_encode($this->patrimoine_reports_model->leads_monthly_report($month));
    }

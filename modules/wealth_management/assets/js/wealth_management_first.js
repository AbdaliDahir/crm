...
$(function () {
    ...
    $('body').on('click', '.milestone-column .cpicker,.milestone-column .reset_milestone_color', function (e) {
        ...
        $.post(admin_url + 'patrimoines/change_milestone_color', {
            color: color,
        ...
        new Dropzone('#patrimoine-files-upload', appCreateDropzoneOptions({
            ...
            success: function (file, response) {
                if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
                    window.location.href = admin_url + 'patrimoines/view/' + patrimoine_id + '?group=patrimoine_files';
                }
            },
    ...
    _table_api = initDataTable('.table-patrimoine-expenses', admin_url + 'patrimoines/expenses/' + patrimoine_id, 'undefined', 'undefined', Expenses_ServerParams, [4, 'desc']);
    ...
    initDataTable('.table-notes', admin_url + 'patrimoines/notes/' + patrimoine_id, [4], [4], 'undefined', [1, 'desc']);
    ...
    initDataTable('.table-timesheets', admin_url + 'patrimoines/timesheets/' + patrimoine_id, [8], [8], Timesheets_ServerParams, [3, 'desc']);
    initDataTable('.table-patrimoine-discussions', admin_url + 'patrimoines/discussions/' + patrimoine_id, undefined, undefined, 'undefined', [1, 'desc']);
    ...
    $('#timesheet select[name="timesheet_task_id"]').on('change', function () {
        ...
        requestGet('patrimoines/timesheet_task_assignees/' + _task_id + '/' + patrimoine_id + '/' + staff_id).done(function (response) {
...
function saveProjectExternalFile(files, externalType) {
    $.post(admin_url + 'patrimoines/add_external_file', {
        files: files,
...
function milestones_switch_view() {
    ...
    if (!$.fn.DataTable.isDataTable('.table-milestones')) {
        initDataTable('.table-milestones', admin_url + 'patrimoines/milestones/' + patrimoine_id);
    }
}
...
function mass_stop_timers(only_billable) {
    requestGetJSON('patrimoines/mass_stop_timers/' + patrimoine_id + '/' + only_billable).done(function (response) {
...
function pre_invoice_patrimoine() {
    requestGet('patrimoines/get_pre_invoice_patrimoine_info/' + patrimoine_id).done(function (response) {
...
function invoice_patrimoine(patrimoine_id) {
    ...
    $.post(admin_url + 'patrimoines/get_invoice_patrimoine_data/', data).done(function (response) {
...
function delete_patrimoine_discussion(id) {
    if (confirm_delete()) {
        requestGetJSON('patrimoines/delete_discussion/' + id).done(function (response) {
...
function view_patrimoine_file(id, $patrimoine_id) {
    ...
    $("#patrimoine_file_data").load(admin_url + 'patrimoines/file/' + id + '/' + patrimoine_id, function (response, status, xhr) {
        if (status == "error") {
...
function update_file_data(id) {
    ...
    $.post(admin_url + 'patrimoines/update_file_data/', data);
}
...
function patrimoine_files_bulk_action(e) {
    if (confirm_delete()) {
        ...
        setTimeout(function () {
            $.post(admin_url + 'patrimoines/bulk_action_files', data).done(function () {
                window.location.reload();
            });
        }, 200);
    }
...
function gantt_filter() {
    ...
    window.location.href = buildUrl(admin_url + 'patrimoines/view/' + patrimoine_id, params);
}
...
function confirm_patrimoine_status_change(e) {
    ...
    $.post(admin_url + 'patrimoines/mark_as', data).done(function (response) {
        response = JSON.parse(response);
...
function milestones_kanban_update(ui, object) {
    if (object === ui.item.parent()[0]) {
        ...
        setTimeout(function () {
            $.post(admin_url + 'patrimoines/update_task_milestone', data)
        }, 50);
    }
}

function milestones_kanban() {
    init_kanban('patrimoines/milestones_kanban', milestones_kanban_update, '.patrimoine-milestone', 445, 360, after_milestones_kanban);
}
...
function after_milestones_kanban() {
    $("#kan-ban").sortable({
        ...
        update: function (event, ui) {
            ...
            $.post(admin_url + 'patrimoines/update_milestones_order', data);
        }
    });

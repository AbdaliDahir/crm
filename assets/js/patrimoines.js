Dropzone.options.patrimoineFilesUpload = false;
Dropzone.options.patrimoineExpenseForm = false;
...
$(function () {
    ...
    // remove the divider for patrimoine actions in case there is no other li except for pin patrimoine
    $('ul.patrimoine-actions li:first-child').next('li.divider').remove();

    var file_id = get_url_param('file_id');
    if (file_id) {
        view_patrimoine_file(file_id, patrimoine_id);
    }

    // Fix for shortcuts in discussions textarea/contenteditable - jquery-comments plugin
    var $discussionsContentEditable = $('#patrimoine_file_data, #discussion-comments');
    ...
    $('body').on('show.bs.modal', '._patrimoine_file', function () {
        discussion_comments('#patrimoine-file-discussion', discussion_id, 'file')
    });

    $('body').on('shown.bs.modal', '._patrimoine_file', function () {
        var content_height = ($('body').find('._patrimoine_file .modal-content').height() - 165);
        var patrimoineFilePreviewIframe = $('.patrimoine_file_area iframe');

        if (patrimoineFilePreviewIframe.length > 0) {
            patrimoineFilePreviewIframe.css('height', content_height);
        }

        if (!is_mobile()) {
            $('.patrimoine_file_area,.patrimoine_file_discusssions_area').css('height', content_height);
        }
    });
    ...
    initDataTable('.table-credit-notes', admin_url + 'credit_notes/table?patrimoine_id=' + patrimoine_id, ['undefined'], ['undefined'], undefined, [0, 'desc']);
    ...
    initDataTable('.table-contracts', admin_url + 'contracts/table?patrimoine_id=' + patrimoine_id, undefined, undefined, ContractsServerParams, [6, 'desc']);

    if ($('#timesheetsChart').length > 0 && typeof (patrimoine_overview_chart) != 'undefined') {
        var chartOptions = {
            type: 'bar',
            data: {},
            options: {
        ...
        chartOptions.data = patrimoine_overview_chart.data;
        var ctx = document.getElementById("timesheetsChart");
        timesheetsChart = new Chart(ctx, chartOptions);
    }
    milestones_kanban();

    $('#patrimoine_top').on('change', function () {
        var val = $(this).val();
        var __patrimoine_group = get_url_param('group');
        if (__patrimoine_group) {
            __patrimoine_group = '?group=' + __patrimoine_group;
        } else {
            __patrimoine_group = '';
        }
        window.location.href = admin_url + 'wealth_management/view/' + val + __patrimoine_group;
    });
    ...
    $('body').on('click', '.milestone-column .cpicker,.milestone-column .reset_milestone_color', function (e) {
        ...
        $.post(admin_url + 'wealth_management/change_milestone_color', {
            color: color,
    ...
    if ($('#patrimoine-files-upload').length > 0) {
        new Dropzone('#patrimoine-files-upload', appCreateDropzoneOptions({
            paramName: "file",
            uploadMultiple: true,
            parallelUploads: 20,
            maxFiles: 20,
            ...
            success: function (file, response) {
                if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
                    window.location.href = admin_url + 'wealth_management/view/' + patrimoine_id + '?group=patrimoine_files';
                }
            },
    ...
    if ($('#patrimoine-expense-form').length > 0) {
        expenseDropzone = new Dropzone("#patrimoine-expense-form", appCreateDropzoneOptions({
    ...
    appValidateForm($('#patrimoine-expense-form'), {
        category: 'required',
        date: 'required',
        amount: 'required',
        currency: 'required'
    }, patrimoineExpenseSubmitHandler);
    ...
    _table_api = initDataTable('.table-patrimoine-expenses', admin_url + 'wealth_management/expenses/' + patrimoine_id, 'undefined', 'undefined', Expenses_ServerParams, [4, 'desc']);

    if (_table_api) {
        _table_api.column(0).visible(false, false).columns.adjust();
    }

    init_rel_tasks_table(patrimoine_id, 'patrimoine');
    initDataTable('.table-notes', admin_url + 'wealth_management/notes/' + patrimoine_id, [4], [4], 'undefined', [1, 'desc']);
    ...
    initDataTable('.table-timesheets', admin_url + 'wealth_management/timesheets/' + patrimoine_id, [8], [8], Timesheets_ServerParams, [3, 'desc']);
    initDataTable('.table-patrimoine-discussions', admin_url + 'wealth_management/discussions/' + patrimoine_id, undefined, undefined, 'undefined', [1, 'desc']);
    ...
    $('#timesheet select[name="timesheet_task_id"]').on('change', function () {
        ...
        requestGet('wealth_management/timesheet_task_assignees/' + _task_id + '/' + patrimoine_id + '/' + staff_id).done(function (response) {
            select_staff.html(response);
            select_staff.selectpicker('refresh');
        });
    });

    $('body').on('change', '#patrimoine_invoice_select_all_tasks,#patrimoine_invoice_select_all_expenses', function () {
...
function patrimoineFileGoogleDriveSave(pickData) {
    saveProjectExternalFile(pickData, 'gdrive');
}
function saveProjectExternalFile(files, externalType) {
    $.post(admin_url + 'wealth_management/add_external_file', {
        files: files,
        ...
        window.location.href = location.split('?')[0] + '?group=patrimoine_files';
    });
}

function milestones_switch_view() {
    $('#milestones-table').toggleClass('hide');
    $('.patrimoine-milestones-kanban').toggleClass('hide');
    if (!$.fn.DataTable.isDataTable('.table-milestones')) {
        initDataTable('.table-milestones', admin_url + 'wealth_management/milestones/' + patrimoine_id);
    }
}
...
function manage_discussion(form) {
    ...
    $.post(url, data).done(function (response) {
        ...
        $('.table-patrimoine-discussions').DataTable().ajax.reload(null, false);
...
function mass_stop_timers(only_billable) {
    requestGetJSON('wealth_management/mass_stop_timers/' + patrimoine_id + '/' + only_billable).done(function (response) {
        alert_float(response.type, response.message);
        setTimeout(function () {
            $('body').find('.modal-backdrop').eq(0).remove();
            init_timers();
            reload_tasks_tables();
            pre_invoice_patrimoine();
        }, 500);
    });
}

function pre_invoice_patrimoine() {
    requestGet('wealth_management/get_pre_invoice_patrimoine_info/' + patrimoine_id).done(function (response) {
        $('#pre_invoice_patrimoine').html(response);
        $('#pre_invoice_patrimoine_settings').modal('show');
    });
}

function invoice_patrimoine(patrimoine_id) {
    $('#pre_invoice_patrimoine_settings').modal('hide');
    var data = {};
    ...
    $.post(admin_url + 'wealth_management/get_invoice_patrimoine_data/', data).done(function (response) {
        $('#invoice_patrimoine').html(response);
        $('#invoice-patrimoine-modal').modal({
            show: true,
            backdrop: 'static'
        });
    });
...
function delete_patrimoine_discussion(id) {
    if (confirm_delete()) {
        requestGetJSON('wealth_management/delete_discussion/' + id).done(function (response) {
...
function patrimoineExpenseSubmitHandler(form) {
...
function view_patrimoine_file(id, $patrimoine_id) {
    $('#patrimoine_file_data').empty();
    $("#patrimoine_file_data").load(admin_url + 'wealth_management/file/' + id + '/' + patrimoine_id, function (response, status, xhr) {
        if (status == "error") {
...
function update_file_data(id) {
    ...
    $.post(admin_url + 'wealth_management/update_file_data/', data);
}

function patrimoine_mark_as_modal(status_id, $patrimoine_id, target) {
    $('#mark_tasks_finished_modal').modal('show');
    $('#patrimoine_mark_status_confirm').attr('data-status-id', status_id);
    $('#patrimoine_mark_status_confirm').attr('data-patrimoine-id', patrimoine_id);
    var $patrimoineMarkedAsFinishedInput = $('#patrimoine_marked_as_finished_email_to_contacts');
    if (status_id == 4) {
        if ($patrimoineMarkedAsFinishedInput.length > 0) {
            $patrimoineMarkedAsFinishedInput.parents('.patrimoine_marked_as_finished').removeClass('hide');
        }
    } else {
        if ($patrimoineMarkedAsFinishedInput.length > 0) {
            $patrimoineMarkedAsFinishedInput.prop('checked', false);
            $patrimoineMarkedAsFinishedInput.parents('.patrimoine_marked_as_finished').addClass('hide');
        }
    }
...
function patrimoine_files_bulk_action(e) {
    if (confirm_delete()) {
        ...
        var rows = $('.table-patrimoine-files').find('tbody tr');
        $.each(rows, function () {
        ...
        setTimeout(function () {
            $.post(admin_url + 'wealth_management/bulk_action_files', data).done(function () {
                window.location.reload();
            });
        }, 200);
    }
...
function gantt_filter() {
    ...
    params['group'] = 'patrimoine_gantt';
    if (status) {
        params['gantt_task_status'] = status;
    }
    window.location.href = buildUrl(admin_url + 'wealth_management/view/' + patrimoine_id, params);
}

function confirm_patrimoine_status_change(e) {
    var data = {};

    $(e).attr('disabled', true);

    data.patrimoine_id = $(e).data('patrimoine-id');
    data.status_id = $(e).data('status-id');

    if (data.status_id == 4) {
        var $patrimoineMarkedAsFinishedInput = $('#patrimoine_marked_as_finished_email_to_contacts');
        if ($patrimoineMarkedAsFinishedInput.length > 0) {
            data.send_patrimoine_marked_as_finished_email_to_contacts = $patrimoineMarkedAsFinishedInput.prop('checked') === true ? 1 : 0;
        }
    }
    ...
    data.notify_patrimoine_members_status_change = $('#notify_patrimoine_members_status_change').prop('checked') === true ? 1 : 0;

    $.post(admin_url + 'wealth_management/mark_as', data).done(function (response) {
        response = JSON.parse(response);
...
function milestones_kanban_update(ui, object) {
    if (object === ui.item.parent()[0]) {
        ...
        setTimeout(function () {
            $.post(admin_url + 'wealth_management/update_task_milestone', data)
        }, 50);
    }
}

function milestones_kanban() {
    init_kanban('wealth_management/milestones_kanban', milestones_kanban_update, '.patrimoine-milestone', 445, 360, after_milestones_kanban);
}
...
function after_milestones_kanban() {
    $("#kan-ban").sortable({
        ...
        update: function (event, ui) {
            ...
            $.post(admin_url + 'wealth_management/update_milestones_order', data);
        }
    });
...
// When marking task as complete if the staff in on patrimoine milestones area, remove this task from milestone in case exists
function _maybe_remove_task_from_patrimoine_milestone(task_id) {
    var $milestonesTasksWrappers = $('.milestone-column');
    if ($("body").hasClass('patrimoine') && $milestonesTasksWrappers.length > 0) {

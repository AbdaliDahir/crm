                     ...
                     <div class="col-md-6">
                        <div class="form-group">
                           ...
                           <select name="rel_type" class="selectpicker" id="rel_type" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                              <option value=""></option>
                              <!-- START MOE#PATRIMOINE-->
                              <option value="patrimoine" <?php if (isset($task) || $this->input->get('rel_type')) {
                                                            if ($rel_type == 'patrimoine') {
                                                               echo 'selected';
                                                            }
                                                         } ?>><?php echo _l('patrimoine'); ?></option>
                              <!-- END MOE#PATRIMOINE-->
         ...
         $('body').on('change', '#rel_id', function() {
            if ($(this).val() != '') {
               if (_rel_type.val() == 'project') {
                  ...
                  if (_rel_type.val() == 'patrimoine') {
                     $.get(admin_url + 'wealth_management/get_rel_patrimoine_data/' + $(this).val() + '/' + taskid, function(patrimoine) {
                        $("select[name='milestone']").html(patrimoine.milestones);
                        if (typeof(_milestone_selected_data) != 'undefined') {
                           $("select[name='milestone']").val(_milestone_selected_data.id);
                           $('input[name="duedate"]').val(_milestone_selected_data.due_date)
                        }
                        $("select[name='milestone']").selectpicker('refresh');
                        if (patrimoine.billing_type == 3) {
                           $('.task-hours').addClass('patrimoine-task-hours');
                        } else {
                           $('.task-hours').removeClass('patrimoine-task-hours');
                        }

                        if (patrimoine.deadline) {
                           var $duedate = $('#_task_modal #duedate');
                           var currentSelectedTaskDate = $duedate.val();
                           $duedate.attr('data-date-end-date', patrimoine.deadline);
                           $duedate.datetimepicker('destroy');
                           init_datepicker($duedate);

                           if (currentSelectedTaskDate) {
                              var dateTask = new Date(unformat_date(currentSelectedTaskDate));
                              var patrimoineDeadline = new Date(patrimoine.deadline);
                              if (dateTask > patrimoineDeadline) {
                                 $duedate.val(patrimoine.deadline_formatted);
                              }
                           }
                        } else {
                           reset_task_duedate_input();
                        }
                        init_patrimoine_details(_rel_type.val(), patrimoine.allow_to_view_tasks);
                     }, 'json');
                  } else {
                     /**END MOE#PATRIMOINE */
                     reset_task_duedate_input();

...
function get_relation_data($type, $rel_id = '', $extra = [])
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if ($rel_id != '') {
            ...
            $where_patrimoines = '';
            if ($CI->input->post('customer_id')) {
                $where_patrimoines .= 'clientid=' . $CI->db->escape_str($CI->input->post('customer_id'));
            }
            $search = $CI->misc_model->_search_patrimoines($q, 0, $where_patrimoines);
            $data   = $search['result'];
        }
        /*END MOE#PATRIMOINE */
    } elseif ($type == 'staff') {
...
function get_relation_values($relation, $type)
{
    ...
    elseif ($type == 'patrimoine') {
        ...
        $link = admin_url('wealth_management/view/' . $id);
    }
    /**END  MOE#PATRIMOINE */
...
function init_relation_options($data, $type, $rel_id = '')
{
    ...
    $has_permission_patrimoines_view  = has_permission('patrimoines', '', 'view'); //MOE#PATRIMOINE
    $is_admin                      = is_admin();
    $CI                            = &get_instance();
    $CI->load->model('projects_model');
    $CI->load->model('patrimoines_model');
    ...
    foreach ($data as $relation) {
        ...
        /**START MOE#PATRIMOINE */
        elseif ($type == 'patrimoine') {
            if (!$has_permission_patrimoines_view) {
                if (!$CI->patrimoines_model->is_member($relation_values['id']) && $rel_id != $relation_values['id']) {
                    continue;
                }
            }
            /**END MOE#PATRIMOINE */

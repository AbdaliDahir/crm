...
/*MOE#PATRIMOINE */
/**
 * Handles upload for patrimoine files
 * @param  mixed $patrimoine_id patrimoine id
 * @return boolean
 */
function handle_patrimoine_file_uploads($patrimoine_id)
{
    $filesIDS = [];
    $errors   = [];
    ...
    if (
        ...
        hooks()->do_action('before_upload_patrimoine_attachment', $patrimoine_id);
        ...
        $path = get_upload_path_by_type('patrimoine') . $patrimoine_id . '/';
        ...
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    ...
                    $CI->db->insert(db_prefix() . 'patrimoine_files', $data);
    ...
    if (count($filesIDS) > 0) {
        ...
        $lastFileID = key($filesIDS);
        $CI->patrimoines_model->new_patrimoine_file_notification($filesIDS[$lastFileID], $patrimoine_id);
    }
